...
def plot_smp_augmented(model, inputs, Map, Weight_func, x1, x2, n_init, *args, **kwargs):
    if model.input_dim == 2:
        plot_smp2D_augmented(model, inputs, n_init, *args, **kwargs)
...
def plot_smp2D_augmented(model, inputs, Map, Weight_func, x1, x2, n_init, filename=None, 
    ...
    plt.contourf(x1,x2,(Map),30,cmap = 'Blues')
    plt.colorbar(label = 'Map')
    # plt.contourf(x1,x2,w,20,cmap = 'Reds',alpha = 0.5)
    plt.contour(x1,x2,Weight_func,20,cmap = 'Reds')
    # plt.clabel(W_plt, inline=1, fontsize=10)
    plt.colorbar(label = 'Weight')
    # plt.legend(['Map','Weight'])
